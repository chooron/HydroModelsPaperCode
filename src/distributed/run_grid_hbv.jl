...
node_params = ComponentVector(params=NamedTuple{keys(merge_params)}([repeat([merge_params[k]], hru_num) for k in keys(merge_params)]))
merge_initstates = ComponentVector(base_pas[:initstates]; s_river=0.0)
node_initstates = ComponentVector(NamedTuple{keys(merge_initstates)}([repeat([merge_initstates[k]], hru_num) for k in keys(merge_initstates)]))
hbv_grid_output = hbv_grid(input_arr, node_params, initstates=node_initstates, config=(timeidx=time_idx, solver=HydroModels.ODESolver()))
