...
result = load("src/implements/save/m50_result.jld2")
obs = result["obs"]
...
function plot_predictions(result)
    # 设置全局字体
    default(fontfamily="calibri", legendfontsize=14,
    ...
    for i in 1:3
        ...
        annotate!(p1[i], 5000, maximum(result["obs"]) * 0.95,
            text(criterion_text, :left, 14, "calibri"))
    end
...
savefig(p, "src/implements/plot/figures/m50_predict.png")