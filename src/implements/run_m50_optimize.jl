...
# m50_opt = HydroModels.GradOptimizer(component=m50_model, solve_alg=Adam(1e-4), adtype=AutoZygote(), maxiters=100)
...
m50_opt_params = load("src/implements/save/m50_opt.jld2")["opt_params"]
...
function loss1(p)
    m50_output = m50_model(input_mat, p, config=config)
    sum(m50_output[end,:] .- flow_vec)
end

@btime gradient(loss1, m50_opt_params)
